...
import { deleteUser, getAllUserList } from "../redux/user/action";
...
const Home = () => {
  ...
  const users = Index.useSelector((state) => state.userReducer.users);
    ...
    if (e.target.value === "") {
      ...
      const result = userData.filter((item) => {
        return (
          ...
          item?.user_email?.toLowerCase().includes(e.target.value?.toLowerCase())||
          item?.user_phone?.toString().toLowerCase().includes(e.target.value?.toLowerCase())
        );
      });
  ...
  return (
    <Index.Box>
      ...
      >
        <Index.Box sx={MODAL_STYLE} className="delete-modal-inner-main">
