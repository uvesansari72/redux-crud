...
import { createUser, getAllUserList, updateUser } from "../redux/user/action";
...
const AddEditUser = (props) => {
  ...
  const onSubmit = (data) => {
    ...
    data.user_password && urlencoded.append("user_password", data.user_password);
    props.row && urlencoded.append("id", props.row?._id);

    if (props.row) {
      dispatch(updateUser(urlencoded));
    } else {
      dispatch(createUser(urlencoded));
    }
  ...
  return (
    <Index.Modal open={props.open} onClose={props.handleClose}>
      <form onSubmit={handleSubmit(onSubmit)}>
        <Index.Box sx={MODAL_STYLE} className="add-edit-modal">
          <Index.Box>
